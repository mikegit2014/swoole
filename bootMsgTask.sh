...
SERVER_PATH=/home/shenruiqing/php-shell
...
case "$1" in
    ...
    start)
            ...
            if [ -n "$PID" ]; then
                ...
                echo -n "$T_FLAG server is running"

                exit 1

            fi

            echo -n "Starting $T_FLAG  server "
    ...
    stop)
            ...
            if [ -z "$PID" ]; then
                ...
                echo -n "$T_FLAG server is not running"

                exit 1

            fi

            echo -n "Gracefully shutting down $T_FLAG server "
    ...
    reload)
            ...
            echo -n "Reload service $T_FLAG server "
